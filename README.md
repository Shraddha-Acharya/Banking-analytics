# 🏦 Indian Banking & Financial Health Analytics

## 🛠️ Tech Stack
Python | SQL | Excel | Tableau | Power BI | Scikit-Learn | Git

## 📌 Overview
End-to-end banking analytics project analyzing Indian banking and financial data to uncover customer behavior, risk patterns and financial health insights.

## 🎯 Business Questions Answered
- Which customers are financially healthy vs at risk?
- What spending patterns predict loan defaults?
- How does income level affect savings behavior?
- Which bank branches perform best?
- What is the financial health score of each customer?

## 📁 Project Structure
| Folder | Contents |
|---|---|
| data/ | Raw datasets |
| notebooks/ | Jupyter notebooks |
| sql/ | SQL queries |
| excel/ | Excel analysis |
| dashboard/ | Power BI + Tableau |
| report/ | Final PDF report |

## 🔑 Key Highlights
- **FinancialHealthScore** — Custom 0-100 scoring system
- **Loan Default Predictor** — ML model
- **Dual Dashboard** — Built in Tableau AND Power BI
- **12 SQL Queries** — Real business intelligence

## 📊 Project Progress
- [x] Phase 1 — Setup and Git ✅
- [ ] Phase 2 — Data Collection
- [ ] Phase 3 — Excel Analysis
- [ ] Phase 4 — SQL Analysis
- [ ] Phase 5 — Python EDA
- [ ] Phase 6 — Feature Engineering
- [ ] Phase 7 — ML Models
- [ ] Phase 8 — Tableau Dashboard
- [ ] Phase 9 — Power BI Dashboard
- [ ] Phase 10 — Final Report

## 👩‍💻 Author
**Shraddha Acharya** — Aspiring Data Analyst
Bengaluru, India
